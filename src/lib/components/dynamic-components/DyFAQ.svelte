...
<div class="flex flex-col gap-5 items-center justify-center p-5 lg:p-10">
    <div class=" rounded-lg p-3  lg:p-5 flex flex-col  pt-10">
        <div class="flex flex-col w-full items-center">
            ...
            <p class="w-2/3 text-sm font-light text-white/80 text-center">{subTitle}</p>
        </div>
    </div>
    <div class="flex flex-col  gap-5 bg-white/10 p-2 lg:p-5 rounded-lg w-full md:w-[80%] lg:w-[70%]">
             ...
             <div class="border-b border-white/20 pb-3">
                <button on:click={()=>{handleChangeShow(id)}} class="  p-2 lg:p-5 text-start flex lg:items-center justify-between gap-3 w-full">
                    <div class="flex lg:items-center gap-3">
                        ...
                        <p class={`${show === id?"text-white" : "text-white/70 "} lg:text-xl hover:text-white`}>
                            {question}
                        </p>

                    </div>
                    <div class={`text-2xl ${show === id?"text-white" : "text-white/70 "}`}>
                        {#if show === id}
                ...
                <p 
                class={`${show === id? " block ":" hidden "}  ml-5 md:ml-10 p-5 bg-black/30 rounded-md text-sm lg:text-base font-light text-white/90`}
                >{answer}</p>
             </div>
        {/each}
    </div>
